...
import { calculateLocalLayeringScore } from "./src/data";
import { getPerfumesList, addPerfumeToDb, autoExpandPerfumeWithAI } from "./src/data/perfumeDb";
...
app.get("/api/perfumes", (req, res) => {
  res.json(getPerfumesList());
});

// AI Auto-Expansion: Search or Register unlisted perfumes in real-time
app.post("/api/perfumes/search-or-create", async (req, res) => {
  const { brand, name } = req.body;
  if (!name) {
    return res.status(400).json({ error: "향수 이름을 입력하세요." });
  }
  ...
  const existing = list.find(
    (p) =>
      p.brand.toLowerCase() === brandStr.toLowerCase() &&
      p.name.toLowerCase() === nameStr.toLowerCase()
  );

  if (existing) {
    return res.json({ perfume: existing, source: "database", isNew: false });
  }

  // Not found in DB, run real-time AI Auto-Expansion using Gemini
  const ai = getGeminiClient();
  if (ai) {
    try {
      const generated = await autoExpandPerfumeWithAI(ai, brandStr, nameStr);
      if (generated) {
        return res.json({ perfume: generated, source: "gemini_ai", isNew: true });
      }
    } catch (err) {
      console.error("[server] AI auto-expansion failed:", err);
    }
  }

  // Fallback if AI is missing or fails: create a simple mock entry
  const fallbackId = `perfume_${Date.now()}`;
  const fallbackPerfume = {
    id: fallbackId,
    brand: brandStr || "Custom Select",
    brandKor: brandStr || "커스텀 스펙",
    name: nameStr,
    nameKor: nameStr,
    scentFamily: ["Fresh"],
    ...
    description: "새로 등록된 시트러스 향수."
  };

  addPerfumeToDb(fallbackPerfume);
  return res.json({ perfume: fallbackPerfume, source: "local_fallback", isNew: true });
});
...
app.post("/api/recommend", async (req, res) => {
  ...
  if (ai) {
    try {
      ...
      const targetPerfume = targetPerfumeId ? getPerfumesList().find(p => p.id === targetPerfumeId) : null;
...
아래 JSON 스키마 규격으로만 완벽한 JSON 문자열로 반환하고 마크다운 백틱 등은 적지 마세요.`;
  ...
  if (targetPerfumeId) {
    const targetP = getPerfumesList().find(p => p.id === targetPerfumeId);
    if (targetP) {
